...
_browser.runtime.onMessage.addListener((message, _, sendResponse) => {
	switch (message.name) {
		case 'downloadSubject':
			storeSubject(message.subject).then(async subjectID => {
...
async function storeSubject(subject) {
	if (!(await checkIfSubjectExists(subject))) return storeData('Subjects', { name: subject });
	else return getKeyOfExistingSubject(subject);
}
